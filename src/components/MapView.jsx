...
import { MapPin, ShieldAlert, CheckCircle2, Search } from 'lucide-react';

export const MapView = () => {
  const { world, player, addResource, addXp } = useGameStore();
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {sectors.map((sector) => (
          <div key={sector.id} className="bg-zinc-950 border border-zinc-800 p-4 rounded-lg hover:border-zinc-700 transition-colors group">
            <div className="flex justify-between items-start mb-4">
              <div className="p-2 bg-zinc-900 rounded group-hover:bg-red-600/20 transition-colors">
                <Search className="w-5 h-5 text-zinc-500 group-hover:text-red-600" />
              </div>
              {sector.danger > 0.5 && <ShieldAlert className="w-4 h-4 text-orange-500" />}
            </div>

            <h4 className="font-bold text-zinc-200">{sector.name}</h4>
            <p className="text-[10px] text-zinc-500 mb-4">{SECTOR_TYPES[sector.type].name}</p>

            <div className="flex gap-2">
              <button
                onClick={() => handleExplore(sector)}
                className="flex-1 bg-red-600 text-white rounded px-4 py-2 hover:bg-red-700 transition-colors text-xs"
              >
                Explore
              </button>
